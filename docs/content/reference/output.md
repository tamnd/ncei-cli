...
describes what users get. Pick a format with `-o`, or let ncei choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ncei <command> -o table   # aligned columns for reading
ncei <command> -o jsonl   # one JSON object per line, for piping
ncei <command> -o json    # a single JSON array
ncei <command> -o csv     # spreadsheet friendly
ncei <command> -o tsv     # tab-separated
ncei <command> -o url     # just the URL column
ncei <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ncei <command> --fields id,title,url
```
...
```bash
ncei <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ncei <command>            # a table, because this is a terminal
ncei <command> | wc -l    # JSONL, because this is a pipe
```
