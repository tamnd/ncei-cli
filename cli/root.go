// Package cli assembles the ncei command tree from the ncei
...
// NewApp assembles the kit application from the ncei domain. The
...
//
// To add a command, declare it in ncei/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ncei.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ncei.Domain{}).Register(app)
